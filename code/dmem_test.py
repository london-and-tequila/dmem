...
def run_exp1(args):
    """Exp 1: D-MEM on standard LoCoMo10 (Table 1)."""
    ...
    for sidx, sample in enumerate(tqdm(samples, desc="Exp1 samples")):
        token_tracker.reset()
        ...
        for r in res:
            if r['metrics']:
                all_metrics_list.append(r['metrics'])
                all_cats.append(r['category'])

        # Log routing stats
        if hasattr(agent.memory_system, 'get_routing_summary'):
            logger.info(f"Sample {sidx} routing: {agent.memory_system.get_routing_summary()}")
        logger.info(f"Sample {sidx} tokens: {token_tracker.snapshot()}")
...
def run_exp2(args):
    """Exp 2: Turn-by-turn scalability (Figure 1a/1b + Table 2)."""
    ...
    for group_name, noise_ratio in groups.items():
        for method in methods:
            ...
            for sidx, sample in enumerate(tqdm(samples, desc=key)):
                ...
                # Routing stats for D-MEM
                if hasattr(agent.memory_system, 'get_routing_summary'):
                    method_logs[-1]['routing'] = agent.memory_system.get_routing_summary()

            all_logs[key] = method_logs
...
def run_exp3(args):
    """Exp 3: Noise robustness (Figure 2)."""
    ...
    for noise_ratio in noise_ratios:
        for method in methods:
            key = f"{method}_noise{int(noise_ratio*100)}"
            logger.info(f"--- {key} ---")
            ...
            for sidx, sample in enumerate(tqdm(samples, desc=key)):
                ...
                res, agg = evaluate_qa(agent, sample.qa, logger)
                method_results.extend(res)
                for r in res:
                    if r['metrics']:
                        all_m.append(r['metrics'])
                        all_c.append(r['category'])

            overall_agg = aggregate_metrics(all_m, all_c) if all_m else {}
            all_results[key] = {
                'noise_ratio': noise_ratio,
                'method': method,
                'aggregate': overall_agg,
                'store_sizes': [],  # filled below if needed
